...
class Text2GraspNetStage1(nn.Module):
...
class Text2GraspNetStage2(nn.Module):
...
class Text2Grasp(nn.Module):
    def __init__(self, input_feature_dim=0, num_view=300, num_angle=12, num_depth=4, cylinder_radius=0.05, hmin=-0.02,
        ...
        super().__init__()
        self.is_training = is_training
        self.view_estimator = Text2GraspNetStage1(input_feature_dim, num_view)
        self.grasp_generator = Text2GraspNetStage2(num_angle, num_depth, cylinder_radius, hmin, hmax_list, is_training)
