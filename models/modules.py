...
class Fusion2(nn.Module):
    ...
    def forward(self, pc_feature, text_features):
        guide_pc = torch.bmm(pc_feature, text_features).transpose(1, 2)
        features = self.cv(guide_pc)
        return features
...
class CosineSimilarity(nn.Module):
    def __init__(self):
        super().__init__()
        self.cosine_sm = nn.CosineSimilarity(dim=1, eps=1e-12)
        self.relu = nn.ReLU()
        self.sm = nn.Softmax(1)
    def forward(self, pc_features, text_features):
        a = torch.bmm(pc_features, text_features)
        norm_pc = F.normalize(pc_features, dim=1)
        norm_text = F.normalize(text_features.transpose(1, 2), dim=1).transpose(1, 2)
        norm1 = torch.bmm(norm_pc, norm_text)
        norm1[norm1==0] = 1e-12
        a = a / norm1
        features = F.normalize(self.relu(a), dim=1)
        alpha = self.sm(features)

        at = torch.bmm(text_features, alpha.transpose(1, 2)).transpose(1, 2)

        features = at * pc_features

        w1 = F.normalize(at, dim=1)
        w2 = F.normalize(pc_features, dim=1)
        # return features
        return features / (w1 * w2).clamp(min=1e-12)
class ApproachNet(nn.Module):
    def __init__(self, num_view, seed_feature_dim):
        ...
        self.conv1 = nn.Conv1d(256, self.in_dim, 1)
