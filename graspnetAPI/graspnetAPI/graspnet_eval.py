...
from .utils.eval_utils import get_scene_name, create_table_points, parse_posevector, load_dexnet_model, \
    transform_points, compute_point_distance, compute_closest_points, voxel_sample_points, topk_grasps, get_grasp_score, \
    collision_detection, eval_grasp
...
class GraspNetEval():
    ...
    **Input:**
    ...
    def get_model(self, scene_id, ann_id, object_id):
        '''
            return models in model coordinate
        '''
        model_dir = os.path.join(self.root, 'models')
        # print('Scene {}, {}'.format(scene_id, camera))
        scene_reader = xmlReader(os.path.join(self.root, 'scenes', scene_id, self.camera, 'annotations',
                                              '%04d.xml' % (ann_id,)))
    ...
    def get_model_poses(self, scene_id, ann_id):
        ...
        **Output:**
        ...
        camera_poses_path = os.path.join(self.root, 'scenes', scene_id, self.camera, 'camera_poses.npy')
        camera_poses = np.load(camera_poses_path)
        camera_pose = camera_poses[int(ann_id)]
        align_mat_path = os.path.join(self.root, 'scenes', scene_id, self.camera, 'cam0_wrt_table.npy')
        align_mat = np.load(align_mat_path)
        # print('Scene {}, {}'.format(scene_id, camera))
        scene_reader = xmlReader(
            os.path.join(scene_dir, scene_id, self.camera, 'annotations', '%04d.xml' % (int(ann_id),)))
        posevectors = scene_reader.getposevectorlist()
        obj_list = []
        pose_list = []
        for posevector in posevectors:
    ...
    def eval_scene(self, scene_id, dump_folder, TOP_K=10, return_list=False, vis=False, max_width=0.1):
        '''
        **Input:**
    ...
    def eval_2(self, dump_folder, scene_id, object_id, img_num, num_text=5, TOP_K=5, max_width=0.1):
        '''
        **Input:**
        ...
        **Output:**
        ...
        model, dexmodel_list, _ = self.get_model(scene_id, ann_id=0, object_id=object_id)

        model_sampled_list = list()

        model_sampled = voxel_sample_points(model[0], 0.008)
        model_sampled_list.append(model_sampled)
        ...
        for i in range(num_text):
            grasp_group = GraspGroup().from_npy(
                os.path.join(dump_folder, scene_id, self.camera, object_id, str(i), f'{img_num}.npy'))
            ...
            # print('\rMean Accuracy for scene:%04d ann:%04d = %.3f' % (
            #     int(scene_id.split('_')[1]), int(ann_id), 100.0 * np.mean(grasp_accuracy, axis=0)), end='', flush=True)

            scene_accuracy.append( np.mean(grasp_accuracy, axis=0))

        # print(np.array(scene_accuracy).shape)
        scene_accuracy = np.mean(np.array(scene_accuracy), axis=0)
        return scene_accuracy

    def parallel_eval_scenes(self, scene_ids, object_id, img_num, dump_folder, proc=2):
        '''
        **Input:**
        ...
        **Output:**
        ...
        res_list = []
        res_list.append(p.apply_async(self.eval_2, (dump_folder, scene_ids, object_id, img_num)))
        p.close()
        p.join()
        scene_acc_list = []
        for res in res_list:
    ...
    def eval_seen(self, dump_folder, scene_ids, object_id, img_num, proc=2, ):
        '''
        **Input:**
        ...
        **Output:**
        ...
        '''
        res = np.array(self.parallel_eval_scenes(scene_ids=scene_ids, object_id=object_id, img_num=img_num, dump_folder=dump_folder, proc=proc))
        ap = np.mean(res)
        if list(res) == []:
            return None
        else:
            ...
            return ap, ap_02, ap_04, ap_06, ap_08, ap_10, ap_12
    ...
    def eval_all(self, dump_folder, proc=2):
        ...
        **Output:**
        ...
        print('\nEvaluation Result:\n----------\n{}, AP={}, AP Seen={}, AP Similar={}, AP Novel={}'.format(self.camera,
                                                                                                           ap[0], ap[1],
                                                                                                           ap[2],
                                                                                                           ap[3]))
        return res, ap
