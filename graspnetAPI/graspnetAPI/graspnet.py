    ...
    def __init__(self, root, camera='kinect', split='train'):
        ...
        **input**:
        ...
        for i in tqdm(self.sceneIds, desc='Loading data path...'):
            objectid_paths = os.path.join(root, f'scene_{str(i).zfill(4)}', camera)
            objectids = os.listdir(objectid_paths)
            for object_id in objectids:
                self.objIds.append(int(object_id))
                img_path = os.path.join(objectid_paths, object_id, '0')
                img_nums = [i.split('.')[0] for i in os.listdir(img_path)]
        ...
        self.objIds = list(set(self.objIds))
